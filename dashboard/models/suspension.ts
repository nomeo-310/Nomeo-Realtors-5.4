...
export interface ISuspensionHistory {
  action: 'suspend' | 'lift' | 'extension' | 'appeal' | 'appeal_approved' | 'appeal_rejected' | 'auto_lift';
  description: string;
...
const SuspensionHistorySchema = new Schema({
  action: {
    type: String,
    required: true,
    enum: ['suspension', 'lift', 'extension', 'appeal', 'appeal_approved', 'appeal_rejected', 'auto_lift']
  },
  description: {
    type: String,
    required: true
