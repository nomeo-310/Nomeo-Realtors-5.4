...
const CustomSelect = ({height, data, placeholder, value, onChange, disabled, style, placeholderStyle, itemText}:Props) => {
  return (
      ...
      <SelectContent className="rounded z-[80000000]">
        <SelectGroup className="z-[800]">
          {data && data.length > 0 && data.map((item:string, index:number) => (
            <SelectItem value={item} className={cn("text-sm lg:text-base rounded", itemText)} key={index}>{item.split('-').join(' ')}</SelectItem>
          ))}
        </SelectGroup>
      </SelectContent>
    </Select>
