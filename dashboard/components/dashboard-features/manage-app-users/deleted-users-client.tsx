...
const DeletedUsersClient = ({user}:{user:AdminDetailsProps}) => {
      ...
      <TableHeader className="rounded-lg h-11 [&_tr]:border-b">
        <TableRow className="bg-white hover:bg-white border-b-0 dark:bg-[#424242]">
          {header.map((item:string, idx: number) => (
            <TableHead className="text-center font-semibold uppercase border-r text-xs last:border-r-0" key={idx}>
              {item}
            </TableHead>
          ))}
        </TableRow>
  ...
  const MobileItem = ({ open, toggleTable, user }: mobileItemProps) => {
    ...
    // You'll need to import and use calculateDeletionStatus here
    const { remainingDays } = user.deletedAt 
      ? calculateDeletionStatus(user.deletedAt) 
      : { remainingDays: 0 };
    ...
    const handleRestoreUser = () => {
      // Add restore user function here
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex flex-col items-end ml-2">
            ...
            <span className="text-xs text-gray-500 dark:text-gray-400">
              {formatDate(user.deletedAt || user.createdAt)}
            </span>
          </div>
        </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="text-right">
                  <p className="text-xs font-medium text-gray-500 dark:text-gray-400">
                    {user.deletedAt ? 'Deleted on' : 'Created on'}
                  </p>
                  <p className="text-sm font-medium dark:text-white">
                    {formatDate(user.deletedAt || user.createdAt)}
                  </p>
            ...
            <div className="space-y-2">
              ...
              {user.role && (
                <div className="flex items-center justify-between py-2 px-1">
                  ...
                  <p className="text-sm dark:text-white capitalize">
                    {user.role}
                  </p>
                </div>
              )}
                ...
                <Table className='w-full border'>
                  ...
                  <TableBody>
                    {users.map((user:ExtendedUserProps, index:number) => (
                      <UserListItem user={user} index={index} key={user._id}/>
                    ))}
                  </TableBody>
                </Table>
...
export default DeletedUsersClient