...
export interface Admin {
  _id: string;
  userId: AdminUser;
  role: 'creator' | 'admin' | 'superAdmin' | 'user' | 'agent'; 
  adminAccess: 'limited_access' | 'full_access' | string;
...
const SuspendedAdminsClient = ({user:current_user}:{user:AdminDetailsProps}) => {
  const router = useRouter();
  const userRole = current_user.role;

  const [currentIndex, setCurrentIndex] = React.useState(-1);

  const fetchData = async (): Promise<AdminList> => {
    try {
      const response = await axios.get<any>('/api/admin/admin/suspended');
  ...
  const { data: admins = [], status } = useQuery<AdminList>({
    queryKey: ['suspended-admins'],
    queryFn: fetchData,
    retry: 2,
    refetchInterval: 15000
  });

  const header = ['S/N', 'Full Name', 'Email', 'Role', 'Admin Access', 'Admin ID', 'Date Joined', 'Suspended On', 'Duration', 'Actions'];
      ...
      <TableHeader className="rounded-lg h-11 [&_tr]:border-b">
        <TableRow className="bg-white hover:bg-white border-b-0 dark:bg-[#424242]">
          {header.map((item: string, idx: number) => (
            <TableHead 
              className="text-center font-semibold uppercase border-r text-xs last:border-r-0 py-3 px-2" 
              key={idx}
            >
              {item}
            </TableHead>
  ...
  const UserListItem = ({user, index}:{user:Admin, index:number}) => {
    const canLiftSuspension = current_user.userId._id === user.userId.suspendedBy;
    ...
    return (
      <TableRow className='border hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors h-11'>
        ...
        <TableCell className="text-xs md:text-sm text-center border-r">
          {user.userId.suspendedAt ? formatDate(user.userId.suspendedAt) : 'N/A'}
        </TableCell>
        <TableCell className="text-xs md:text-sm text-center border-r">
          {user.userId.suspensionDuration || 'Indefinite'}
        </TableCell>
        <TableCell className="text-xs md:text-sm text-center">
          <Menu user={user} canLiftSuspension={canLiftSuspension}/>
        </TableCell>
      </TableRow>
    )
  };

  const Menu = ({ user, canLiftSuspension }: { user: Admin; canLiftSuspension: boolean }) => {
    ...
    const handleLiftSuspension = () => {
      router.push(`/${userRole === 'superAdmin' ? 'superadmin' : userRole}-dashboard/manage-admins/suspended/${user.userId._id}`);
    };
      ...
      <DropdownMenu modal={false} open={showMenu} onOpenChange={setShowMenu}>
        ...
        <DropdownMenuContent className="w-64 min-w-[200px] z-50" align="end">
          ...
          <div className="p-2 border-t border-gray-100">
            ...
            <DropdownMenuItem 
              className="flex items-center gap-3 px-3 py-2 text-sm cursor-pointer rounded-md transition-colors text-blue-600 focus:text-blue-600 focus:bg-blue-50 hover:bg-blue-50 mb-1"
              onClick={() => {
                handleViewDetails();
                setShowMenu(false);
              }}
        ...
        {open && (
          <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-600 space-y-4">
            ...
            <div className="p-3 rounded-lg bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-800/30">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium dark:text-white">Admin Account Suspended</p>
                  <p className="text-xs text-gray-600 dark:text-gray-400 mt-1">
                    {canLiftSuspension 
                      ? 'You suspended this admin account'
                      : 'This admin account has been suspended'}
                  </p>
                </div>
                <div className="text-right">
                  <p className="text-xs font-medium text-gray-500 dark:text-gray-400">Admin ID</p>
                  <p className="text-sm font-medium dark:text-white lowercase">
                    {user.adminId.toLowerCase()}
                  </p>
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <p className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Suspended For</p>
              ...
              <div>
                <p className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Suspended On</p>
                <p className="text-sm font-medium dark:text-white">
                  {user.userId.suspendedAt ? formatDate(user.userId.suspendedAt) : 'N/A'}
                </p>
              </div>
            ...
            <div className="pt-3 border-t border-gray-200 dark:border-gray-600">
              <div className="grid grid-cols-2 gap-2">
                ...
                <button 
                  ...
                  className={cn("px-3 py-2.5 bg-red-50 text-red-600 dark:bg-red-900/20 dark:text-red-400 rounded-lg hover:bg-red-100 dark:hover:bg-red-900/30 transition-colors flex items-center justify-center gap-2 text-sm font-medium", canLiftSuspension ? 'col-span-2' : 'col-span-2')}
                >
                  <Trash2 className="w-4 h-4" />
                  Delete Admin
                </button>
...
export default SuspendedAdminsClient;