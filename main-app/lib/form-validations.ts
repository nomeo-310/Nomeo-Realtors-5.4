...
const auth = z.object({
  ...
  confirm_password: z
    .string()
    .min(1, { message: "Confirm your password please!!" }),
  ...
  phoneNumber: z
    ...
    .refine(validatePhoneNumber, {
      message: "Phone number should be 11 digits and must be a valid one",
    }),
...
const profile = z.object({
  ...
  phoneNumber: required_string.refine(validatePhoneNumber, {
    message: "Phone number should be 11 digits and must be a valid one",
  }),
  ...
  address: required_string.min(1, {
    message: "Your current residence address is required",
  }),
  userBio: z.string().optional(),
  agencyName: required_string.min(1, {
    message: "Name of your agency is required",
  }),
  agencyAddress: required_string.min(1, {
    message: "City of residence is required",
  }),
  inspectionFeePerHour: z.coerce.number({
    message: "Your inpection fee per hour must be a valid number",
  }),
  officeNumber: required_string.refine(validatePhoneNumber, {
    message: "Phone number should be 11 digits and must be a valid one",
  }),
});
...
export const signupSchema = auth
  .pick({
    email: true,
    password: true,
    username: true,
...
export const restoreSchema = auth.pick({
  email: true,
  password: true,
  username: true,
});
...
export const sendOtpSchema = auth
  .pick({ otp: true, password: true, confirm_password: true })
  .refine((data) => data.password === data.confirm_password, {
    message: "Password does not match",
    path: ["confirm_password"],
  });
...
export const resetPasswordSchema = auth
...
export const faqMessageSchema = auth.pick({
  message: true,
  email: true,
  name: true,
});
...
export const agentProfileSchema = profile
  .pick({
    surName: true,
    lastName: true,
    city: true,
    ...
    agentBio: true,
    inspectionFeePerHour: true,
    officeNumber: true,
    agencyAddress: true,
    agencyName: true,
  })
  .refine((data) => data.phoneNumber !== data.officeNumber, {
...
export const userProfileSchema = profile
  .pick({
    surName: true,
    lastName: true,
    city: true,
...
export const transactionSchema = auth.pick({
  email: true,
  phoneNumber: true,
  amount: true,
});
...
export const contactSchema = auth.pick({
  fullName: true,
  phoneNumber: true,
  email: true,
  message: true,
...
export const addApartmentSchema = z.object({
  propertyTag: required_string.min(1, {
    message: "Property tag is important!!",
  }),
  ...
  facilityStatus: required_string.min(1, {
    message: "Facility status is required!!",
  }),
  monthlyRent: z.coerce
    .number({ message: "Monthly rent must be a valid number" })
    .optional(),
  propertyPrice: z.coerce
    .number({ message: "Property price must be a valid number" })
    .optional(),
  annualRent: z.coerce
    .number({ message: "Annual rent must be a valid number" })
    .optional(),
  bedrooms: required_string
    .min(1, { message: "Number of bedrooms is required" })
    .regex(/^\d+$/, { message: "Must contain only numbers" }),
  bathrooms: required_string
    .min(1, { message: "Number of bathrooms is required" })
    .regex(/^\d+$/, { message: "Must contain only numbers" }),
  toilets: required_string
    .min(1, { message: "Number of toilet is required" })
    .regex(/^\d+$/, { message: "Must contain only numbers" }),
  squareFootage: required_string
    .min(1, { message: "The size of is required" })
    .regex(/^\d+$/, { message: "Must contain only numbers" }),
});
export type addApartmentValues = z.infer<typeof addApartmentSchema>;

export const scheduleInspectionSchema = z.object({
  date: z.date({ message: "This is not a date!" }),
...
export const createBlogSchema = z.object({
  title: required_string.min(1, { message: "Title is required!!" }),
  bannerImageUrl: required_string.min(1, {
