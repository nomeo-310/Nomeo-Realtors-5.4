...
import { toggleInspectionVerdict } from "@/actions/inspection-actions";
...
export const useToggleInspectionVerdict = () => {
  const queryClient = useQueryClient();

  const mutation = useMutation({
    mutationFn: (data: ToggleInspectionStatusData) => toggleInspectionVerdict(data),
    onMutate: async (data) => {
        ...
        const updateInspectionInArray = (inspections: Inspection[]) => 
          inspections.map((inspection: Inspection) => 
            inspection._id === data.id 
              ? { ...inspection, verdict: data.verdict }
              : inspection
          );
