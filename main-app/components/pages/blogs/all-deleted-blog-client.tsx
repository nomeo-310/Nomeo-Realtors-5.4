...
import { fullPostDelete, restoreDeletedPost } from '@/actions/blog-actions';
import { HugeiconsIcon } from '@hugeicons/react';

const AllDeletedBlogClient = ({user}:{user:userProps}) => {
  ...
  if (query) {
    url = `/api/blog/all-user-deleted?page=${page}&query=${query}`;
  } else {
    url = `/api/blog/all-user-deleted?page=${page}`;
  }

  const request = () => axios.get(url);

  const { data, status } = useQuery({
    queryKey: ['all-user-delete', page, query],
  ...
  const createSearchParam = (searchText:string) => {
    if (searchText) {
      ...
      router.push(`/${user.role === 'superAdmin' ? 'admin' : user.role === 'creator' ? 'admin' : user.role}-dashboard/created-blogs/deleted?${params.toString()}`);
    } else {
      router.push(`/${user.role === 'superAdmin' ? 'admin' : user.role === 'creator' ? 'admin' : user.role}-dashboard/created-blogs/deleted`);
    }  
  };
  ...
  const BlogCard = ({blog}:{blog:Blog}) => {
    ...
    const deleteBlog = async (blogId:string) => {
      await fullPostDelete(blogId).then((response) => {
        if (response.status === 200) {
          queryClient.invalidateQueries({queryKey: ['blog-counts']});
          queryClient.invalidateQueries({queryKey: ['all-user-delete']});
    ...
    const restoreBlog = async (blogId:string) => {
      await restoreDeletedPost(blogId).then((response) => {
        if (response.status === 200) {
          queryClient.invalidateQueries({queryKey: ['blog-counts']});
          queryClient.invalidateQueries({queryKey: ['all-user-delete']});
            ...
            <div className="w-full flex items-center justify-between text-white dark:text-white/80">
              ...
              <DropdownMenu open={showMenu} onOpenChange={setShowMenu}>
                ...
                <DropdownMenuContent className='bg-white dark:bg-neutral-900 min-w-[5rem] -ml-12 z-[400]'>
                  <DropdownMenuItem className='cursor-pointer text-xs py-1' onClick={() => restoreBlog(blog._id)}>
                    Restore
                  </DropdownMenuItem>
...
export default AllDeletedBlogClient