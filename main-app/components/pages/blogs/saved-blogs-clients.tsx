...
import { Blog, userBlogData } from '@/lib/types';
...
const SavedBlogsClient = () => {
  ...
  const url = `/api/blog/saved-blogs?page=${currentPage}`;
  ...
  const BlogList = () => {
    ...
    if (status === 'success' && allUserBlogs && allUserBlogs.blogs.length < 1) {
      return (
        <div className="md:w-[65%] mx-auto py-12">
          <EmptyState message={query ? `No Blog found for this keyword: ${query}` : 'You have not bookmarked any blog.'} className='' />
        </div>
      )
    };
