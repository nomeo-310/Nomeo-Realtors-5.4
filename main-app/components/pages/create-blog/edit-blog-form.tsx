...
import { ArrowUpRight03Icon, Cancel01Icon, Delete01Icon, UserAdd01Icon, User03Icon } from "@hugeicons/core-free-icons";
...
import { publishDraft, updateDraft } from "@/actions/blog-actions";
...
const EditBlogForm = ({blog, user}:{blog:BlogPost, user:userProps}) => {
  ...
  const defaultBlogValues = {
    title: blog.title || '',
    bannerImageUrl: blog.banner?.secure_url || '',
    bannerImagePublicId: blog.banner?.public_id || '',
  ...
  const [uploadedImageUrl, setUploadedImageUrl] = React.useState<string | null>(blog.banner?.secure_url || null);
  ...
  const [isCollaborating, setIsCollaborating] = React.useState(blog.collaboration || false);
  const [collaborators, setCollaborators] = React.useState<string[]>(blog.collaborators || []);
  ...
  React.useEffect(() => {
    if (isCollaborating && collaborators.length === 2) {
      setOpenInput(false);
    } else {
      setOpenInput(true);
    }
  ...
  React.useEffect(() => {
    if (initialCollaborators && initialCollaborators.length > 0) {
      if (initialCollaborators.length === 2) {
        setOpenInput(false);
      }
    }
  ...
  React.useEffect(() => {
    const fetchCollaboratorDetails = async () => {
      try {
        const response = await axios.post('/api/user/get-collaborators', {
          contributorIds: blog.collaborators,
        });
        setCollaboratorDetails(response.data);
      } catch (error) {
        toast.error('Failed to load collaborator details');
      }
    };
    if (blog.collaborators?.length > 0) {
  ...
  const removeCollaborator = (userId:string) => {
    if (initialCollaborators.includes(userId)) {
      toast.error('Cannot remove original collaborators');
      return;
    }
  ...
  const createBlog = async (data: createBlogValues) => {
    ...
    try {
      ...
      await publishDraft(fulldata).then((response) => {
  ...
  const createDraft = async () => {
    ...
    try {
      setIsSaving(true);
      await updateDraft(fulldata).then((response) => {
        if (response.success && response.status === 200) {
          setClearContent(true);
          setPreviewImage(null);
          form.reset();
          ...
          toast.success(response.message);
          queryClient.invalidateQueries({queryKey: ['blog-counts']})
          queryClient.invalidateQueries({queryKey: [ isACollaborator ? 'all-user-collaborations' : 'all-user-drafts']})
          isACollaborator ? router.push(`/${user.role === 'superAdmin' ? 'admin' : user.role === 'creator' ? 'admin' : user.role}-dashboard/created-blogs/collaboration`) 
  ...
  const CollaboratorCard = ({detail, selector}:{detail:searchedUserDetail, selector: boolean}) => {
    ...
    const isInitialCollaborator = initialCollaborators.includes(_id);
      ...
      <div className="flex items-center justify-between border-b p-2 last:border-0">
        <div className="flex gap-2 items-center">
          {profilePicture ?
            ...
            <div className="size-10 md:size-11 lg:size-12 rounded-full flex items-center justify-center relative bg-secondary-blue text-white">
              {surName && lastName ? <p className="text-sm lg:text-base text-center font-semibold tracking-widest">{surName[0] + lastName[0]}</p> : <HugeiconsIcon icon={User03Icon} className="size-5 md:size-6 lg:size-7" />}
            </div>
          }
          <div className="">
        ...
        {selector ? (
          !alreadySelected && (
            <button
              className="flex items-center gap-2 md:p-2 p-1.5 md:px-4 px-3 rounded-full bg-black text-white text-sm"
              onClick={() => addCollaborator(detail)}
            >
              Select
            </button>
          )
        ) : (
          !isInitialCollaborator && (
            <button
              className="flex items-center gap-2 md:p-2 p-1.5 md:px-4 px-3 rounded-full bg-black text-white text-sm"
              onClick={() => removeCollaborator(_id)}
            >
              Remove
            </button>
          )
      ...
      <form  className="w-full flex-col flex gap-3 md:gap-4 md:w-[90%] xl:w-[80%] 2xl:w-[75%]"  onSubmit={form.handleSubmit(createBlog)}>
        ...
        <div className="w-full">
          <div className="w-full rounded-lg xl:rounded-xl xl:h-[450px] md:h-[350px] lg:h-[400px] bg-[#d4d4d4] dark:bg-[#404040] aspect-video md:aspect-auto xl:mt-3 mt-2 relative overflow-hidden cursor-pointer">
            ...
            {!uploadedImageUrl && !previewImage && !imageLoading && (
          ...
          render={({ field }) => (
            <FormItem>
              <FormControl className="pt-3">
                <TipTap onChange={field.onChange} clearContent={clearContent} content={blog.content} edit />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
        ...
        <div className="flex-none flex md:gap-4 justify-between w-full md:w-auto">
          {blog.is_published &&
            <React.Fragment>
              <button className="text-sm lg:text-base xl:py-2.5 xl:px-6 md:py-2 md:px-5 py-2 px-4 rounded-full bg-black dark:bg-[#424242] dark:border dark:border-[#424242] hover:bg-black/60 text-white font-medium flex gap-2 items-center">
                {isPublishing ? "Updating post..." : "Update Post"}
                {isPublishing && (
          ...
          { blog.is_draft &&
            <React.Fragment>
              { isACollaborator ?
                <React.Fragment>
                  <button
                    className="text-sm lg:text-base xl:py-2.5 xl:px-6 md:py-2 md:px-5 py-2 px-4 rounded-full bg-[#d4d4d4] dark:border-white/60 dark:border dark:bg-inherit hover:bg-[#9c9c9c] font-medium flex items-center gap-2"
                    type="button"
                    onClick={createDraft}
                  >
                ...
                <React.Fragment>
                  ...
                  <button
                    ...
                    onClick={createDraft}
                  >
                    {isSaving ? "Saving as draft..." : "Save as Draft"}
                    {isSaving && <Loader2 className="animate-spin xl:size-6 size-5" />}
                  </button>
                </React.Fragment>
              }
