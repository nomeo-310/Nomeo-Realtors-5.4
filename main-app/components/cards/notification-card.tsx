...
import { Cancel01Icon, HourglassIcon, AlertCircleIcon, SecurityCheckIcon, CreditCardAcceptIcon, Clock03Icon, Note01Icon, TelephoneIcon, ArrowRight01Icon, Link05Icon, CreditCardAddIcon, QuillWrite02Icon, User03Icon, ContactBookIcon } from '@hugeicons/core-free-icons';
...
const NotificationCard = ({notification, user, index}:notificationCardProps) => {
  const { seen, type, _id, createdAt, title, content, issuer, recipient, propertyId, blogId } = notification;
  ...
  const { mutate } = useDeleteNotification(_id);
  ...
  return (
    ...
    >
      ...
      <div className="flex-1">
            ...
            { user.role === 'agent' ?
              <div className='w-full mt-2 flex gap-2 flex-col'>
                  ...
                  <div className='flex gap-2 items-center'>
                    ...
                    <div className="flex flex-col gap-1">
                      <div className="flex items-center gap-2">
                        ...
                        <h2 className='text-sm text-black/70 font-medium'>{issuer?.firstName} {issuer?.lastName}</h2>
                      </div>
                      <div className='flex items-center gap-2'>
                ...
                <div className='mt-2 flex items-center justify-end'>
                  <button className='rounded-lg text-sm text-red-500'>Cancel Inspection</button>
                </div>
              </div> 
        ...
        { type === 'payment' &&
          <Link href={user.role === 'user' ? `/user-dashboard/transactions?propertyId=${propertyId}&agentUserId=${issuer?._id}`: '/agent-dashboard/transactions'} className='text-black/60 w-full mt-2 flex gap-2 justify-end items-center text-sm lg:text-base dark:text-white/70'>
            Make payment
