...
export const GET = async (req:Request) => {
  ...
  try {
    ...
    const properties = await Apartment.find({_id: {$in: current_user.likedApartments}})
    ...
    const totalProperties = await Apartment.countDocuments({_id: {$in: current_user.likedApartments}});
