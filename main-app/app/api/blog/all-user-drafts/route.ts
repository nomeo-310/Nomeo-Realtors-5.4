...
export const GET = async (req: Request) => {
  ...
  try {
    ...
    if (query === '') {
      searchQuery = { author: current_user._id, is_draft: true, is_deleted: false};
    } else {
      searchQuery = { author: current_user._id, is_draft: true, is_deleted: false, $or: [{ title: new RegExp(query, 'i')},{ description: new RegExp(query, 'i') }]};
    };
