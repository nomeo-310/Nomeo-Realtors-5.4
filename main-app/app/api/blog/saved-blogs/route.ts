...
export const GET = async (req: Request) => {
  ...
  try {
    ...
    if (query === '') {
      searchQuery = { _id: { $in: current_user.bookmarkedABlogs }};
    } else {
      searchQuery = { _id: { $in: current_user.bookmarkedABlogs }, $or: [{ title: new RegExp(query, 'i')},{ description: new RegExp(query, 'i') }]};
    };
